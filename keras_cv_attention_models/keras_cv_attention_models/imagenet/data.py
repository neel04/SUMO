...
spec = importlib.util.spec_from_file_location("SUMO_dataset", "/home/awesome/scripts/sumo_dataset.py")
foo = importlib.util.module_from_spec(spec)
sys.modules["SUMO_dataset"] = foo
...
def random_crop_and_resize_image(image, target_shape, scale=(0.08, 1.0), ratio=(0.75, 1.3333333), method="bilinear", antialias=False):
    ...
    tf.print(">>>> BEFORE RESIZE:", image.shape)
    ...
    tf.print(">>>> AFTER RESIZE:", image.shape)
...
class RandomProcessDatapoint:
    ...
    def img_resizer(self, image):
        ...
        print(f'>>>> image shape: {image.shape} and {(*self.target_shape[:2], channel)} -> {len(image.shape)} dims')
        assert image.shape == (*self.target_shape[:2], channel), 'Image shapes are incorrect: Got {}, expected {}'.format(image.shape, (*self.target_shape[:2], channel))
    ...
    def __call__(self, datapoint, token_label=None):
        image = datapoint[0] #batch_size x 963 x 256 x 256 x 6
        label = datapoint[1]

        return tf.map_fn(self.img_resizer, image, dtype=tf.float32), label
...
def evaluation_process_crop_resize(datapoint, target_shape=(224, 224), central_crop=1.0, resize_method="bilinear", antialias=False):
    ...
    image = tf.map_fn(lambda x: tf.image.resize(x, target_shape, method=resize_method, antialias=antialias), image, dtype=tf.float32)
    label = datapoint[1]
    return image, label
...
def xx_reshaper(x):
    '''
    Reshape a dynamic tensor with unknown 'None' dimensions
    '''
    return tf.reshape(x, [-1, *x.shape[2:]])

def yy_reshaper(y):
    '''
    Reshape a dynamic tensor with unknown 'None' dimensions
    input shape: [None, 963, 51]
    output shape: [None, 51]
    '''
    return tf.reshape(y, [-1, y.shape[2]])

def is_test(x, y):
    return x % 10 == 0 # 90% split
...
def init_dataset(
    ...
    num_layers=2,
    use_positional_related_ops=True,
    use_shuffle=False, #True, shuffling disabled for now
    seed=None,
    ...
    **augment_kwargs,  # Too many...
    ...
    if data_name.endswith(".json"):
        ...
        dataset = foo.SUMO_dataset(AUTOTUNE)
        num_classes = 51 #hardcoded for now
        num_channels = 6 #info.features["image"].shape[-1]
        total_images = 52488
        eval_cutoff = 4500
    ...
    """ Train dataset """
    #train_dataset = dataset.take(total_images-eval_cutoff) #.skip(eval_cutoff)
    train_dataset = dataset.enumerate().filter(is_train, name='filtering_train').map(recover, num_parallel_calls=AUTOTUNE)
    if use_token_label:
    ...
    train_dataset = train_dataset.map(lambda *x: train_pre_batch(x), num_parallel_calls=AUTOTUNE) #.batch(batch_size, drop_remainder=is_tpu)
    ...
    if use_token_label:
        ...
        train_post_batch = lambda xx, yy: ((xx_reshaper(xx) - mean) / std, yy_reshaper(yy))
        train_post_batch = lambda xx, yy: ((xx - mean) / std, yy)
    ...
    """ Test dataset """
    #test_dataset = dataset.skip(total_images-eval_cutoff)
    test_dataset = dataset.enumerate().filter(is_test, name='filtering_test').map(recover, num_parallel_calls=AUTOTUNE)
    if test_dataset is not None:
        ...
        test_dataset = test_dataset.map(lambda *x: test_pre_batch(x), num_parallel_calls=AUTOTUNE)
        ...
        if use_token_label:
            ...
            test_post_batch = lambda xx, yy: ((xx_reshaper(xx) - mean) / std, yy_reshaper(yy), num_classes)
