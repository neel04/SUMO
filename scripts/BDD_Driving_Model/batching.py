...
tf.app.flags.DEFINE_boolean('use_MIMO_inputs_pipeline', True,
  ...
  with tf.name_scope('batch_processing'):
    ...
    if FLAGS.use_MIMO_inputs_pipeline:
        ...
        for thread_id in range(num_preprocess_threads):
            ...
            datan.append(net_inputs + net_outputs) ##(6, 32) (6, 32, 6) (6, 32, 2)

        # the single thread batch_join dequeue_many operation might be the bottleneck.
        if net_inputs[0].get_shape()[0] == batch_size:
            ...
            for i in range(FLAGS.num_batch_join):
                ''' This is where the magic happens, to reduce the batch size to its correct shapes
                '''
