...
def read_one_video(video_path, jobid):
    ...
    hz_res = 1 if FLAGS.low_res else 30 #15 -> 30
    ratio = False

    # save the speed field
    #updates path to latest BDD100K file structure
    json_base = os.path.dirname(fd).replace('videos', 'info/100k/train')

    json_path = os.path.join(json_base, fprefix+".json") #(json_base, "info", fprefix+".json")
    speeds = get_interpolated_speed(json_path, fprefix+".mov", hz_res)
    logging.debug('Interpolation successfull')
    if speeds is None:
        # if speed is none, the error message is printed in other functions
        return 0, False

    if speeds.shape[0] < FLAGS.truncate_frames:
        print("skipping since speeds are too short!:", speeds.shape[0])
        return 0, False
    ...
    if FLAGS.low_res:
        ...
        logging.debug('Entering High Res if/else')
    ...
    if FLAGS.low_res:
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            'image/height': _int64_feature(512),
            'image/width': _int64_feature(512),
            'image/channel': _int64_feature(3),
            'image/class/video_name':_bytes_feature([video_path]),
            'image/format':_bytes_feature(['JPEG']),
            'image/encoded': _bytes_feature(image_list),
        ...
        logging.debug('TF Example Successfully created!')
...
def convert_one(video_path, jobid):
    ...
    if not os.path.exists(out_name):
        ...
        logging.debug('Read Ended')
        if state:
            writer = tf2.io.TFRecordWriter(out_name) #tf.python_io.TFRecordWriter(out_name)
...
def p_convert(video_path_list, jobid):
    ...
    for video_path in video_path_list:
        logging.debug('|--- STARTING {}'.format(len(video_path_list)))
        fd, fprefix, cache_images, out_name = parse_path(video_path, jobid)
